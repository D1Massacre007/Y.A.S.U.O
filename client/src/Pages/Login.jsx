...
import Loading from './Loading'; // import if you want to show it explicitly

const Login = () => {
  const { setUser, setLoading } = useAppContext(); // <-- added setLoading
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // For demo purposes, just set a dummy user
    setUser({ name: mode === "login" ? "Demo User" : name || "Demo User", email });

    // Show loading for a few seconds
    if (setLoading) {
  ...
  return (
    <div className="w-full max-w-sm rounded-lg bg-[#1f1c2c] border border-gray-700 p-4 mx-2 text-white">
      ...
      <h1 className="mb-4 text-center text-2xl font-semibold text-purple-400">
        {mode === "Y.A.S.U.O" ? "Y.A.S.U.O" : "Y.A.S.U.O"}
      </h1>
      ...
      <p className="mt-4 text-center text-gray-400">
        {mode === "login" ? (
          <>Don't have an account? <span onClick={() => setMode("register")} className="text-purple-400 cursor-pointer">Sign Up</span></>
        ) : (
          <>Already have an account? <span onClick={() => setMode("login")} className="text-purple-400 cursor-pointer">Login</span></>
        )}
      </p>
